function preprocessing_reaching_NEW
...
namefolder=['Reaching preprocessing subject NEW' subject];
mkdir(namefolder);
...
% Data loading
cd(['E:\Data\NewData_EEG_EMG_grasping\New subjects\Subject ', subject, '\Reaching preprocessing subject ', subject])
load('reaching_original.mat')
...
% Data filtering
disp('Filtering data...');
reaching_filtered=reaching_ch_removed1;
reaching_filtered.data = double(reaching_filtered.data);
...
reaching_resampled=pop_resample(reaching_filtered,sr_new);        
...
reaching_ch_removed2=reaching_resampled;
...
for ii=1:n_trials
    ...
    for nn=1:n_channels
%                 data_res(nn,:)=ricampiona(block.data(nn,...
%                     block_start:block_end),n_frames);
        data_res(nn,:)=reaching_IC_visual_removal1.data(nn,...
    ...
    event=struct('type',['trial_' num2str(trials(ii))],'latency',...
        (block_start_prev),'duration',size(block_start:block_end,2)+...
        preframes,'init_index',trials(ii),'init_time',NaN,'urevent',...
        trials(ii));
    urevent=struct('type',['trial_' num2str(trials(ii))],'latency',...
        (size(block_start:block_end,2)+preframes)*(trials(ii)-1)+1,'duration',size(block_start:block_end,2)+...
        preframes,'init_index',trials(ii),'init_time',NaN);
    events=struct([events event]);
    urevents=struct([urevents urevent]);
    block_start_prev = block_start_prev+size(block_start:block_end,2)+preframes;
end
...
for nn=1:length(bad_trials)
    st=(reaching_concatenated.event(bad_trials(nn)).latency);
    rm_data=[rm_data st:st+reaching_trials_visual_removal.event(bad_trials(nn)).duration-1];
end
reaching_trials_visual_removal.data(:,rm_data)=[];
reaching_trials_visual_removal.pnts=size(reaching_trials_visual_removal.data,2);
reaching_trials_visual_removal.xmax=(reaching_trials_visual_removal.pnts-1)/sr_new;
reaching_trials_visual_removal.times=linspace(reaching_trials_visual_removal.xmin,reaching_trials_visual_removal.xmax,size(reaching_trials_visual_removal.data,2));
reaching_trials_visual_removal.event=reaching_trials_visual_removal.event(good_trials);
reaching_trials_visual_removal.urevent=reaching_trials_visual_removal.urevent(good_trials);
reaching_trials_visual_removal.event(1).latency = 1;
previous_event = reaching_trials_visual_removal.event(1).latency + reaching_trials_visual_removal.event(1).duration;
for nn=2:length(good_trials)


    reaching_trials_visual_removal.event(nn).latency = previous_event;
    previous_event = reaching_trials_visual_removal.event(nn).latency + reaching_trials_visual_removal.event(nn).duration;
    reaching_trials_visual_removal.event(nn).init_index=nn;
    reaching_trials_visual_removal.event(nn).urevent=nn;
    reaching_trials_visual_removal.urevent(nn).latency=reaching_trials_visual_removal.event(nn).latency;
    reaching_trials_visual_removal.urevent(nn).init_index=reaching_trials_visual_removal.event(nn).init_index;
end
data=reaching_trials_visual_removal;
eegplot(data.data,'srate',sr_new,'events',data.event);
save('reaching_trials_visual_removal_new.mat','reaching_trials_visual_removal');
...
save('reaching_ICA2_new.mat','reaching_ICA2');
...
save('reaching_IC_visual_removal2_new.mat','reaching_IC_visual_removal2');
...
save('reaching_dipole_new.mat','reaching_dipole');
...
save('reaching_IC_visual_removal3_new.mat','reaching_IC_visual_removal3');
...
%     /sr_new]);
data=reaching_epoched;
data.event(1).latency = 1;
previous_event = data.event(1).latency + data.event(1).duration;

for ep = 2:size(data.event,2)
    data.event(ep).latency = previous_event;
    previous_event = data.event(ep).latency + data.event(ep).duration;
       
end

for ep = 1:size(data.event,2)
    if ep == size(data.event,2)
        dataepoch{ep} = data.data(:,data.event(ep).latency:data.event(ep).latency+data.event(ep).duration-1);
    else
        dataepoch{ep} = data.data(:,data.event(ep).latency:data.event(ep).latency+data.event(ep).duration-1);
    end
end
...
for ep = 1:size(data.event,2)
    for chan = 1:size(dataepoch{1},1)
        dataepoch{ep}(chan,:) = dataepoch{ep}(chan,:) - mean(dataepoch{ep}(chan,1:preframes));
    end
end

epoched = dataepoch;
save('reaching_epoched_new.mat','epoched');
...
for ep = 1:size(epoched,2)
    data.data = [data.data,epoched{ep}];
end
eegplot(data.data,'srate',sr_new,'events',data.event);
  
bad_epochs=input('Bad epochs (vector): ');
for ep = 1:size(bad_epochs,2)
    epoched{bad_epochs(ep)} = [];
        
end
k = 1;
for ep = 1:size(epoched,2) 
    if ~isempty(epoched{ep})
        epoched2{k} = epoched{ep};
        k = k+1;
    end
end
 
 save('epoched_corr_new.mat', 'epoched2')
