function output=order_by_cond_MAY2017(data_matrix,conditions,n_trial)
...
for j = 1:2 %step (baseline, post)
    ...
    temp_data = squeeze(data_matrix(:,:,j)); % timepoints, trials, step
    for i = 1:n_trial %trials
        if conditions(i,1,j)==10
            if conditions(i,2,j)==0
                data_cond(:,count4,4,j) =  temp_data(:,i); % timepoints, trials (per cond), cond, step
                count4=count4+1;
